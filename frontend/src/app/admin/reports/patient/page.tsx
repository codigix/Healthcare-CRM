...
export default function PatientStatisticsPage() {
  ...
  const columns = [
    ...
    { header: 'Total Patients', accessor: 'totalpatients', sortable: true },
    { header: 'New Registrations', accessor: 'newregistrations', sortable: true },
    { header: 'Follow-ups', accessor: 'followups', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    reportid: `${'Patient Statistics'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    category: `Sample ${'Patient Statistics'} ${i + 1}`,
    totalpatients: `Sample Data ${i + 1}`,
    newregistrations: `Sample Data ${i + 1}`,
    followups: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3 my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Patient demographics and stats.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">New Patients (M)</p>
          <p className="text-2xl  mt-2 text-blue-400">1,240</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Retention Rate</p>
          <p className="text-2xl  mt-2 text-emerald-400">86%</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Avg Age</p>
          <p className="text-2xl  mt-2 text-purple-400">42</p>
        </div>
      </div>
      ...
      <div className="my-3">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center my-3 my-3 bg-dark-tertiary/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search patient statistics..."
              value={searchTerm}
