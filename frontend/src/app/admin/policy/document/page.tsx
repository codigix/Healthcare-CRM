...
export default function DocumentControlPage() {
  ...
  const columns = [
    ...
    { header: 'Changes', accessor: 'changes', sortable: true },
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    docid: `${'Document Control'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    documentname: `Sample ${'Document Control'} ${i + 1}`,
    currentversion: `Sample Data ${i + 1}`,
    changes: `Sample Data ${i + 1}`,
    modifiedby: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Version history and document tracking.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">In Circulation</p>
          <p className="text-2xl  mt-2 text-emerald-400">720</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Archived</p>
          <p className="text-2xl  mt-2 text-gray-400">120</p>
        </div>
      </div>
