...
export default function PolicyAcknowledgementPage() {
  ...
  const columns = [
    { header: 'Ack ID', accessor: 'ackid', sortable: true },
    { header: 'Employee Name', accessor: 'employeename', sortable: true },
    { header: 'Policy Name', accessor: 'policyname', sortable: true },
    { header: 'Sent Date', accessor: 'sentdate', sortable: true },
    { header: 'Signed Date', accessor: 'signeddate', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    ackid: `${'Policy Acknowledgement'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    employeename: `Sample ${'Policy Acknowledgement'} ${i + 1}`,
    policyname: `Sample Data ${i + 1}`,
    sentdate: `Sample Data ${i + 1}`,
    signeddate: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Track staff policy adherence.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Pending Signs</p>
          <p className="text-2xl  mt-2 text-amber-400">142</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Compliance Rate</p>
          <p className="text-2xl  mt-2 text-emerald-400">94%</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Overdue</p>
          <p className="text-2xl  mt-2 text-red-400">28</p>
        </div>
      </div>
