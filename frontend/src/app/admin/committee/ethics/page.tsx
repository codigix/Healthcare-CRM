...
export default function EthicsCommitteePage() {
  ...
  const columns = [
    ...
    { header: 'Primary Investigator', accessor: 'primaryinvestigator', sortable: true },
    { header: 'Submission Date', accessor: 'submissiondate', sortable: true },
    { header: 'Review Status', accessor: 'reviewstatus', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    caseid: `${'Ethics Committee'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    subject: `Sample ${'Ethics Committee'} ${i + 1}`,
    primaryinvestigator: `Sample Data ${i + 1}`,
    submissiondate: `Sample Data ${i + 1}`,
    reviewstatus: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Medical ethics and clinical trials review.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active Trials</p>
          <p className="text-2xl  mt-2 text-indigo-400">12</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
