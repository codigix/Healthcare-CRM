...
export default function CommitteeManagementPage() {
  ...
  const columns = [
    { header: 'Committee ID', accessor: 'committeeid', sortable: true },
    { header: 'Name', accessor: 'name', sortable: true },
    { header: 'Chairperson', accessor: 'chairperson', sortable: true },
    { header: 'Members', accessor: 'members', sortable: true },
    { header: 'Next Meeting', accessor: 'nextmeeting', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    committeeid: `${'Committee Management'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    name: `Sample ${'Committee Management'} ${i + 1}`,
    chairperson: `Sample Data ${i + 1}`,
    members: `Sample Data ${i + 1}`,
    nextmeeting: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3 my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Hospital committees and meetings.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active Committees</p>
          <p className="text-2xl  mt-2 text-blue-400">18</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Meetings This Week</p>
          <p className="text-2xl  mt-2 text-purple-400">4</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Pending Actions</p>
          <p className="text-2xl  mt-2 text-amber-400">22</p>
        </div>
      </div>
      ...
      <div className="my-3">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center my-3 my-3 bg-dark-tertiary/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search committee management..."
              value={searchTerm}
