...
export default function SafetyCommitteePage() {
  ...
  const columns = [
    { header: 'Inspection ID', accessor: 'inspectionid', sortable: true },
    { header: 'Area', accessor: 'area', sortable: true },
    { header: 'Inspector', accessor: 'inspector', sortable: true },
    { header: 'Date', accessor: 'date', sortable: true },
    { header: 'Hazard Level', accessor: 'hazardlevel', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    inspectionid: `${'Safety Committee'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    area: `Sample ${'Safety Committee'} ${i + 1}`,
    inspector: `Sample Data ${i + 1}`,
    date: `Sample Data ${i + 1}`,
    hazardlevel: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Workplace and patient safety standards.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Open Hazards</p>
