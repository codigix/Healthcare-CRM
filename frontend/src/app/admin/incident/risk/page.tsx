...
export default function RiskManagementPage() {
  ...
  const columns = [
    { header: 'Risk ID', accessor: 'riskid', sortable: true },
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    riskid: `${'Risk Management'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    description: `Sample ${'Risk Management'} ${i + 1}`,
    department: `Sample Data ${i + 1}`,
    probability: `Sample Data ${i + 1}`,
    impact: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Proactive risk identification and mitigation.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active Risks</p>
          <p className="text-2xl  mt-2 text-amber-400">14</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Critical Risks</p>
          <p className="text-2xl  mt-2 text-red-400">1</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Mitigated</p>
