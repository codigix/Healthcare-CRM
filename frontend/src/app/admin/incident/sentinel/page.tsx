...
export default function SentinelEventsPage() {
  ...
  const columns = [
    ...
    { header: 'Date Occurred', accessor: 'dateoccurred', sortable: true },
    { header: 'Reported To Auth', accessor: 'reportedtoauth', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    eventid: `${'Sentinel Events'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    eventtype: `Sample ${'Sentinel Events'} ${i + 1}`,
    patientid: `Sample Data ${i + 1}`,
    dateoccurred: `Sample Data ${i + 1}`,
    reportedtoauth: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Tracking and analysis of critical sentinel events.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Total Events YTD</p>
          <p className="text-2xl  mt-2 text-red-400">2</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Under Investigation</p>
          <p className="text-2xl  mt-2 text-amber-400">1</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Closed</p>
          <p className="text-2xl  mt-2 text-emerald-400">1</p>
        </div>
      </div>
