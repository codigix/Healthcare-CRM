...
export default function CAPATrackingPage() {
  ...
  const columns = [
    { header: 'CAPA ID', accessor: 'capaid', sortable: true },
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    capaid: `${'CAPA Tracking'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    source: `Sample ${'CAPA Tracking'} ${i + 1}`,
    actionrequired: `Sample Data ${i + 1}`,
    assignedto: `Sample Data ${i + 1}`,
    deadline: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Corrective and Preventive Actions.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Verified Closed</p>
