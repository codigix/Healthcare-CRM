          ...
          <div className="card">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-dark-tertiary">
                    <th className="py-3 px-4 text-gray-400 font-medium">Issue ID</th>
                    <th className="py-3 px-4 text-gray-400 font-medium">Recipient</th>
                    <th className="py-3 px-4 text-gray-400 font-medium">Blood Type</th>
                    <th className="py-3 px-4 text-gray-400 font-medium">Doctor</th>
                    <th className="py-3 px-4 text-gray-400 font-medium">Dept</th>
                ...
                <tbody>
                  {loadingIssues ? (
                    <tr>
                      <td colSpan={7} className="text-center py-4 text-gray-400">Loading blood issuances...</td>
                    </tr>
                  ) : bloodIssues.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="text-center py-4 text-gray-500">No blood issue requests logged in system.</td>
                    </tr>
                  ) : (
                    bloodIssues.slice(0, 5).map((issue) => (
                      <tr key={issue.id} className="border-b border-dark-tertiary hover:bg-dark-tertiary/50 transition-colors">
                        <td className="py-3 px-4 font-mono text-xs text-gray-400">{issue.issueId}</td>
                        <td className="py-3 px-4 font-semibold text-white">{issue.recipient}</td>
                        <td className="py-3 px-4">
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                            issue.bloodType.includes("+") ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"
                          }`}>
                            {issue.bloodType} ({issue.units} Unit{issue.units > 1 ? "s" : ""})
                          </span>
                        </td>
                        <td className="py-3 px-4 text-gray-300">{issue.requestingDoctor}</td>
                        <td className="py-3 px-4 text-gray-400">{issue.department}</td>
                        <td className="py-3 px-4">
                          <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                            issue.status === "Approved" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" :
