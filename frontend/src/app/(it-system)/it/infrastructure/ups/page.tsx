...
export default function UPSMonitoringPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { header: 'UPS Unit', accessor: 'upsunit', sortable: true },
    { header: 'Location', accessor: 'location', sortable: true },
    { header: 'Current Load', accessor: 'currentload', sortable: true },
    { header: 'Battery Level', accessor: 'batterylevel', sortable: true },
    { header: 'Est. Runtime', accessor: 'estruntime', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Uninterruptible Power Supply loads and battery health in server rooms.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-dark-secondary rounded p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Total UPS Load</p>
          <p className={"text-2xl font-bold mt-2 " + "text-blue-400"}>45 kVA</p>
        </div>
        <div className="bg-dark-secondary rounded p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">All Batteries</p>
          <p className={"text-2xl font-bold mt-2 " + "text-emerald-400"}>Healthy</p>
        </div>
        <div className="bg-dark-secondary rounded p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active on UPS</p>
