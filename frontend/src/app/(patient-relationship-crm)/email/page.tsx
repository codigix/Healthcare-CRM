...
export default function EmailPage() {
    ...
    const [emails, setEmails] = useState<EmailMessage[]>([
        ...
        },
        {
            ...
            preview:
                "You need additional documentation for claim #87654. Please provide the requested information within 14 days.",
            date: "over 2 years ago",
            read: false,
            starred: true,
            hasAttachment: true,
        },
        {
            ...
            preview:
                "Dear colleague, I'm referring a patient for cardiology consultation. Patient records attached.",
            date: "over 2 years ago",
            read: true,
            starred: false,
            hasAttachment: true,
            ...
            <div className="flex gap-6 h-full">
                <div className="w-64 border-r border-dark-tertiary">
                        ...
                        {folders.map((folder) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between p-2 rounded transition-colors ${activeFolder === folder.id
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        <Icon size={15} />
                                        <span className="text-mdfont-medium">{folder.label}</span>
                                    </div>
                                    <span
                                        className={`text-xs font-semibold ${activeFolder === folder.id
                                            ? "text-emerald-500"
                                            : "text-gray-500"
                                            }`}
                                    >
                    ...
                    <div className="mt-6 pt-6 border-t border-dark-tertiary">
                        ...
                        <div className="space-y-1 px-2">
                            {["Patient", "Admin", "Lab", "Pharmacy", "Insurance"].map(
                                (label) => (
                                    <button
                                        key={label}
                                        className="w-full text-left p-2 rounded text-mdtext-gray-400 hover:bg-dark-tertiary transition-colors"
                                    >
                                        • {label}
                                    </button>
                                )
                    ...
                    <div className="p-6 space-y-4">
                        ...
                        <div className="space-y-2">
                            {emails.map((email) => (
                                <div
                                    key={email.id}
                                    className={`p-4 rounded border border-dark-tertiary hover:bg-dark-tertiary/50 transition-colors cursor-pointer ${!email.read ? "bg-dark-secondary" : ""
                                        }`}
                                >
                                    <div className="flex items-start justify-between mb-2">
                                        <div className="flex items-center gap-2 flex-1">
                                            ...
                                            <div>
                                                <p
                                                    className={`font-medium ${!email.read ? "text-white" : "text-gray-300"
                                                        }`}
                                                >
                                                    {email.from}
                                                </p>
                                                <p
                                                    className={`text-md${!email.read
                                                ...
                                                <p className="text-mdtext-gray-500 mt-1">
                                                    {email.preview}
                                                </p>
                                            </div>
                                        </div>
                        ...
                        <div className="text-mdtext-gray-500 mt-4">Page 1 of 1</div>
                    </div>
                </div>
            </div>
        </>
