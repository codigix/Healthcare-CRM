...
export default function SettingsPage() {
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    settingid: `${'Settings'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    module: `Sample ${'Settings'} ${i + 1}`,
    key: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3 my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Admin module settings.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          ...
          <p className="text-2xl  mt-2 text-emerald-400">Healthy</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Last Backup</p>
          <p className="text-2xl  mt-2 text-blue-400">2 hrs ago</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active Users</p>
          <p className="text-2xl  mt-2 text-purple-400">142</p>
        </div>
      </div>
