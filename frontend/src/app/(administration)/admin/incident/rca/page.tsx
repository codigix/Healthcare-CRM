...
export default function RootCauseAnalysisPage() {
  ...
  const columns = [
    ...
    { header: 'Related Incident', accessor: 'relatedincident', sortable: true },
    { header: 'Lead Investigator', accessor: 'leadinvestigator', sortable: true },
    { header: 'Started', accessor: 'started', sortable: true },
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    rcaid: `${'Root Cause Analysis'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    relatedincident: `Sample ${'Root Cause Analysis'} ${i + 1}`,
    leadinvestigator: `Sample Data ${i + 1}`,
    started: `Sample Data ${i + 1}`,
    duedate: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">In-depth investigation of incidents.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active RCAs</p>
