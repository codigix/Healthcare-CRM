...
export default function PatientAdministrationPage() {
  ...
  const columns = [
    ...
    { header: 'Blood Group', accessor: 'bloodgroup', sortable: true },
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    patientid: `${'Patient Administration'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    name: `Sample ${'Patient Administration'} ${i + 1}`,
    agegender: `Sample Data ${i + 1}`,
    bloodgroup: `Sample Data ${i + 1}`,
    lastvisit: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3 my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Manage patient records and demographics.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Registered Patients</p>
          <p className="text-2xl  mt-2 text-blue-400">45k+</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active IPD</p>
          <p className="text-2xl  mt-2 text-emerald-400">850</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Today OPD</p>
          <p className="text-2xl  mt-2 text-purple-400">1,240</p>
        </div>
      </div>
      ...
      <div className="my-3">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center my-3 my-3 bg-dark-tertiary/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search patient administration..."
              value={searchTerm}
