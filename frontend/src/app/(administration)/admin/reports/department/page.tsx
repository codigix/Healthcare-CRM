...
export default function DepartmentPerformancePage() {
  ...
  const columns = [
    ...
    { header: 'Patients Seen', accessor: 'patientsseen', sortable: true },
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    reportid: `${'Department Performance'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    department: `Sample ${'Department Performance'} ${i + 1}`,
    patientsseen: `Sample Data ${i + 1}`,
    revenue: `Sample Data ${i + 1}`,
    avgwaittime: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3 my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Departmental KPIs and metrics.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          ...
          <p className="text-2xl  mt-2 text-blue-400">Cardiology</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Avg Wait Time</p>
          <p className="text-2xl  mt-2 text-emerald-400">18 mins</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Total Revenue</p>
          <p className="text-2xl  mt-2 text-purple-400">$145k</p>
        </div>
      </div>
      ...
      <div className="my-3">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center my-3 my-3 bg-dark-tertiary/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search department performance..."
              value={searchTerm}
