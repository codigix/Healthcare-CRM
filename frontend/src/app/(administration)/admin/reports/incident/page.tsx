...
export default function IncidentReportsPage() {
  ...
  const columns = [
    ...
    { header: 'Total Incidents', accessor: 'totalincidents', sortable: true },
    { header: 'Resolved', accessor: 'resolved', sortable: true },
    { header: 'Pending', accessor: 'pending', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    reportid: `${'Incident Reports'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    period: `Sample ${'Incident Reports'} ${i + 1}`,
    totalincidents: `Sample Data ${i + 1}`,
    resolved: `Sample Data ${i + 1}`,
    pending: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3 my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Detailed incident reporting.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Critical Incidents</p>
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Avg Resolution Time</p>
          <p className="text-2xl  mt-2 text-blue-400">4 hrs</p>
        </div>
      </div>
