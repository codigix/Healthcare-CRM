...
export default function PharmacyCommitteePage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { header: 'Agenda ID', accessor: 'agendaid', sortable: true },
    { header: 'Topic', accessor: 'topic', sortable: true },
    { header: 'Proposed By', accessor: 'proposedby', sortable: true },
    { header: 'Review Date', accessor: 'reviewdate', sortable: true },
    { header: 'Decision', accessor: 'decision', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    agendaid: `${'Pharmacy Committee'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    topic: `Sample ${'Pharmacy Committee'} ${i + 1}`,
    proposedby: `Sample Data ${i + 1}`,
    reviewdate: `Sample Data ${i + 1}`,
    decision: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Drug formulary and pharmacy governance.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Formulary Items</p>
          <p className="text-2xl  mt-2 text-blue-400">1,420</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Recent Approvals</p>
