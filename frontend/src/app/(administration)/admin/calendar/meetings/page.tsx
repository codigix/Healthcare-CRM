...
export default function MeetingsPage() {
  ...
  const columns = [
    ...
    { header: 'Room', accessor: 'room', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => (
  ...
  const mockData = Array.from({ length: 12 }).map((_, i) => ({
    meetingid: `${'Meetings'.substring(0, 3).toUpperCase()}-${1000 + i}`,
    subject: `Sample ${'Meetings'} ${i + 1}`,
    chairperson: `Sample Data ${i + 1}`,
    datetime: `Sample Data ${i + 1}`,
    room: `Sample Data ${i + 1}`,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Inter-departmental and board meetings.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Meetings Today</p>
          <p className="text-2xl  mt-2 text-amber-400">4</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">This Week</p>
          <p className="text-2xl  mt-2 text-blue-400">18</p>
        </div>
        <div className="bg-dark-secondary rounded4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Canceled</p>
