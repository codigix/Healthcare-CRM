...
export default function PendingPrescriptionsPage() {
  ...
  const columns_1: Column<any>[] = [
    ...
    { header: "Required Qty", accessor: "qty" },
    {
      header: "Stock Available", accessor: (item) => (<>\n{item.isUnregistered ? (
    ...
    { header: "Tax Rate", accessor: (item) => (<>{item.taxRate}\n%</>) },
    { header: "Line Total", accessor: (item) => (<>₹\n{item.lineTotal.toFixed(2)}</>) },
  ];

  const columns_0: Column<any>[] = [
    {
      header: "Patient Name", accessor: (pres) => (<>\n<div className="flex items-center gap-2">
        <div className="w-9 h-9 bg-gradient-to-br from-[#1abc9c]/20 to-[#1abc9c]/5 text-[#1abc9c] rounded-xl flex items-center justify-center  border border-[#1abc9c]/20">
          <User size={16} />
        </div>
        <div>
    ...
    {
      header: "Doctor Name", accessor: (pres) => (<>\n<div className="text-sm">
    ...
    {
      header: "Status", accessor: (pres) => (<>\n<span className="px-2.5 py-1 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-full text-[11px]  uppercase tracking-wider">
        Awaiting Dispense
      </span>\n</>)
    },
    {
      header: "Action", accessor: (pres) => (<>\n<button
        onClick={() => setSelectedPrescription(pres)}
        className="btn-primary py-1.5 px-4  text-xs flex items-center gap-2 inline-flex justify-center shadow-lg shadow-emerald-950/20 hover:shadow-emerald-500/10 transition-all rounded"
      >
        <Eye size={14} /> View
      </button>\n</>)
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-dark-secondary p-6 rounded-2xl border border-white/5 shadow-2xl">
        <div className="flex items-center gap-4">
          <Link
            ...
            className="p-3 bg-card/5 hover:bg-[#1abc9c]/20 hover:text-[#1abc9c] rounded-xl transition-all duration-300"
          >
            <ArrowLeft size={20} />
          </Link>
          <div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {/* Search bar */}
          <div className="relative bg-card/5 border border-white/5 focus-within:border-[#1abc9c]/50 rounded-xl px-3.5 py-2 flex items-center gap-2 w-64 transition-all">
          ...
          <select
            ...
            className="bg-[#181920] border border-white/5 hover:border-white/10 text-gray-300 text-sm rounded-xl p-2 outline-none cursor-pointer transition-all"
          >
          ...
          <button
            ...
            className="p-2.5 bg-card/5 border border-white/5 hover:border-[#1abc9c]/30 hover:bg-[#1abc9c]/10 text-gray-300 hover:text-[#1abc9c] rounded-xl flex items-center justify-center transition-all duration-300 shadow-sm"
            title="Refresh Queue"
          >
            <RefreshCw size={15} />
          </button>
      ...
      {successMsg && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-xl p-4 flex items-center gap-2 animate-fadeIn">
      ...
      {error && (
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl flex items-center gap-2 animate-fadeIn">
      ...
      <div className="card overflow-hidden border border-white/5 shadow-2xl p-0">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-xl  text-white mb-2">Prescription Queue Clean</h3>
          ...
          <>
            <div className="overflow-x-auto">
              <DataTable columns={columns_0} data={paginatedPrescriptions} enableLocalSearch enableLocalPagination />
            </div>

            {/* Pagination Controls */}
            {totalPages > 1 && (
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 bg-card/[0.01] border-t border-white/5">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="p-1.5 bg-card/5 hover:bg-card/10 disabled:hover:bg-card/5 text-gray-300 disabled:text-gray-600 disabled:opacity-50 rounded border border-white/5 disabled:border-transparent transition-all cursor-pointer disabled:cursor-not-allowed"
                    title="Previous Page"
                  >
                    <ChevronLeft size={16} />
                  </button>
                  ...
                  {Array.from({ length: totalPages }, (_, index) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs  rounded transition-all cursor-pointer ${isCurrent
                          ? "bg-[#1abc9c]/10 text-[#1abc9c] border border-[#1abc9c]/30 shadow-md shadow-[#1abc9c]/5"
                          : "bg-card/5 hover:bg-card/10 text-gray-300 border border-white/5"
                          }`}
                      >
                        {pageNum}
                      </button>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-[#13141b] border border-white/10 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl relative shadow-emerald-500/5 transition-all">
            ...
            <div className="space-y-2">
              {/* Info Columns */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-card/5 p-4 rounded-xl border border-white/5">
                <div>
                  <span className="text-gray-500 text-xs uppercase  tracking-wider block">Patient</span>
                  <span className="text-white  text-sm block mt-1">{selectedPrescription.patient?.name}</span>
                ...
                <div>
                  <span className="text-gray-500 text-xs uppercase  tracking-wider block">Flow & Date</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-[#181920]/50 p-4 rounded-xl border border-white/5">
                  <span className="text-amber-400  text-xs uppercase tracking-wider block">Pharmacist Notes / Directives</span>
              ...
              <div className="space-y-2">
                ...
                <div className="border border-white/5 rounded-xl overflow-hidden bg-card/[0.01]">
                  <DataTable columns={columns_1} data={modalDetails.items} enableLocalSearch enableLocalPagination />
                </div>
              </div>
              ...
              {(modalDetails.hasStockError || modalDetails.missingMedicine) && (
                <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl flex items-center gap-2">
                  ...
                  <div>
                    <span className=" text-sm block">Dispensing Lockout Activated</span>
              ...
              <div className="flex flex-col md:flex-row items-start justify-between gap-6 pt-4 border-t border-white/5">
                <div className="text-xs text-gray-400 space-y-1 w-full md:max-w-md">
                  <p className=" text-gray-300">Dispensation & Billing Note:</p>
                ...
                <div className="bg-card/5 rounded-xl p-4 border border-white/5 w-full md:w-64 space-y-2.5">
                  ...
                  <div className="flex justify-between items-center my-3 my-3 text-sm  pt-1">
            ...
            <div className="p-6 border-t border-white/5 bg-card/[0.01] flex justify-end gap-3 rounded-b-2xl">
              ...
              <button
                ...
                className="btn-primary py-2.5 px-6 rounded-xl  text-sm flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {dispenseLoading ? (
                  <>
