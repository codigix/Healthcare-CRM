...
export default function PatientProfilePage() {
  ...
  if (error || !patient) {
    return (
      <div className="card max-w-lg mx-auto mt-12 p-8 text-center space-y-6">
        ...
        <h2 className="text-2xl  text-white">Error Loading Profile</h2>
        ...
        <button onClick={() => router.push("/patients")} className="btn-primary w-full flex items-center justify-center gap-2">
          <ArrowLeft size={15} /> Back to Patients Registry
        </button>
      </div>
    );
  }
  ...
  const columns_1: Column<any>[] = [
    ...
    { header: "Created Date", accessor: (inv) => (<>{formatDate(inv.date)}</>) },
    { header: "Due Date", accessor: (inv) => (<>{formatDate(inv.dueDate)}</>) },
    { header: "Amount", accessor: (inv) => (<>$\n{parseFloat(inv.amount).toFixed(2)}</>) },
    {
      header: "Status", accessor: (inv) => (<>\n<span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${inv.status === "paid" ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"
        }`}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/patients" className="p-2 hover:bg-dark-tertiary rounded transition-colors border border-dark-tertiary">
            <ArrowLeft size={20} />
          </Link>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-3xl ">{patient.name}</h1>
              <span className={`px-3 py-1 rounded-full text-xs font-semibold ${patient.status === "Inactive" ? "bg-red-500/20 text-red-400" : "bg-emerald-500/20 text-emerald-400"
                }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <div className="card col-span-1 space-y-6 border-emerald-500/20 shadow-md">
          <div className="flex flex-col items-center text-center space-y-3 pb-6 border-b border-dark-tertiary">
            <div className="w-20 h-20 rounded-full bg-emerald-500/10 border-2 border-emerald-500/30 flex items-center justify-center text-emerald-500 text-3xl ">
          ...
          <div className="space-y-4 text-sm">
            <div className="flex justify-between items-center my-3 my-3 bg-dark-tertiary/30 p-2.5 rounded border border-dark-tertiary/20">
              <span className="text-gray-400 flex items-center gap-2"><Droplet size={16} className="text-red-500" /> Blood Group</span>
              <span className=" text-white bg-red-500/10 px-2 py-0.5 rounded text-xs">{patient.bloodGroup || "Not Specified"}</span>
            </div>
            ...
            {tabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-5 py-4 font-medium text-sm transition-all flex items-center gap-2 border-b-2 whitespace-nowrap ${activeTab === tab.id
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold mb-4 text-emerald-400 flex items-center gap-2">
                    <Activity size={15} /> Clinical Background & Metadata
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-dark-secondary/40 p-5 rounded border border-dark-tertiary/50">
                    <div>
                      <h4 className="text-xs  text-gray-400 uppercase tracking-wider mb-1">Registration Date</h4>
                      <p className="text-white font-medium">{formatDate(patient.createdAt)}</p>
                    </div>
                    <div>
                      <h4 className="text-xs  text-gray-400 uppercase tracking-wider mb-1">Last Update Date</h4>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-dark-tertiary/30 border border-dark-tertiary p-5 rounded text-gray-300 leading-relaxed min-h-[120px]">
                    {patient.history ? (
                ...
                {appointments.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {appointments.map((apt) => (
                      <div key={apt.id} className="p-5 bg-dark-secondary/50 border border-dark-tertiary/50 rounded hover:border-emerald-500/20 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold tracking-wider ${apt.status === "completed"
                  ...
                  <div className="space-y-6">
                    {prescriptions.map((pr) => {
                      ...
                      const columns_0: Column<any>[] = [
                        ...
                        { header: "Dosage", accessor: (med) => (<>{med.dosage || "N/A"}</>) },
                        { header: "Frequency", accessor: (med) => (<>{med.frequency || "N/A"}</>) },
                        { header: "Duration", accessor: (med) => (<>{med.duration || "N/A"}</>) },
                      ];


                      return (
                        <div key={pr.id} className="p-6 bg-dark-secondary/50 border border-dark-tertiary rounded space-y-4">
                          ...
                          <div>
                            <span className="text-xs  text-gray-400 uppercase tracking-wider block mb-2">Medications</span>
                            <div className="overflow-x-auto">
                              <DataTable columns={columns_0} data={medicationsList} enableLocalSearch enableLocalPagination />
                            </div>
                          </div>

                          {pr.notesForPharmacist && (
                            <div className="bg-dark-tertiary/30 p-3 rounded border border-dark-tertiary/50">
                              <span className="text-[10px]  text-gray-400 uppercase tracking-wider block">Instruction for Pharmacist</span>
                ...
                {labs.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {labs.map((lab) => {
                      ...
                      return (
                        <div key={lab.id} className="p-6 bg-dark-secondary/50 border border-dark-tertiary rounded space-y-4">
                          <div className="flex justify-between items-start gap-4 border-b border-dark-tertiary/40 pb-3">
                            ...
                            <span className={`px-2.5 py-1 text-xs  rounded border ${isCompleted
                          ...
                          {isCompleted && (
                            <div className="space-y-3">
                              <div>
                                <span className="text-[10px]  text-gray-400 uppercase tracking-wider block">Clinical Observations & Findings</span>
                                <p className="text-xs text-gray-300 bg-dark-tertiary/20 p-3 rounded border border-dark-tertiary/20 mt-1 leading-relaxed whitespace-pre-line">
 ...
 `);
                                      ...
                                      className="p-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded text-xs  flex items-center justify-center gap-1.5 transition-colors shadow shadow-emerald-500/10"
                                    >
                                    ...
                                    <a
                                      ...
                                      className="p-2 bg-dark-tertiary hover:bg-dark-tertiary/70 text-white rounded text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border border-dark-tertiary/50"
                                    >
                                      <Download size={13} />
                                      Download
                                    </a>
                              ...
                              {parsed.fileData && parsed.fileType?.startsWith("image/") && (
                                <div className="border border-dark-tertiary rounded p-2 bg-dark-secondary/30 flex items-center justify-center max-h-[220px] overflow-hidden w-full sm:w-60">
                                  <img
                                    src={parsed.fileData}
            ...
            {activeTab === "billing" && (
              <div className="space-y-6">
                ...
                {invoices.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <DataTable columns={columns_1} data={invoices} enableLocalSearch enableLocalPagination />
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === "admissions" && (
              <div className="space-y-6">
                ...
                {admissions.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {admissions.map((adm) => (
                      <div key={adm.id} className="p-5 bg-dark-secondary/50 border border-dark-tertiary/50 rounded space-y-3">
                        <div className="flex flex-wrap justify-between items-center border-b border-dark-tertiary/50 pb-3 gap-2">
                          <div className="flex items-center gap-2">
                            <span className=" text-white">Room {adm.roomNumber || "N/A"}</span>
                            <span className="text-xs text-gray-400">{adm.roomType || "Standard"} • {adm.department || "General"}</span>
                          </div>
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${adm.status === "Occupied" ? "bg-amber-500/10 text-amber-400" : "bg-emerald-500/10 text-emerald-400"
                            }`}>
                            {adm.status}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                          <div>
                            <span className="text-gray-400 block uppercase  tracking-wider">Attending Doctor</span>
                            <span className="text-white font-medium mt-0.5 block">{adm.attendingDoctor || "Physician"}</span>
                          </div>
                          <div>
                            <span className="text-gray-400 block uppercase  tracking-wider">Allotment Date</span>
                            <span className="text-white font-medium mt-0.5 block">{formatDate(adm.allotmentDate)}</span>
                          </div>
                          {adm.expectedDischargeDate && (
                            <div>
                              <span className="text-gray-400 block uppercase  tracking-wider">Discharge Date</span>
                              <span className="text-white font-medium mt-0.5 block">{formatDate(adm.expectedDischargeDate)}</span>
                            </div>
                          )}
                          {adm.insuranceDetails && (
                            <div>
                              <span className="text-gray-400 block uppercase  tracking-wider">Insurance Cover</span>
                        ...
                        {adm.additionalNotes && (
                          <div className="bg-dark-tertiary/30 p-2.5 rounded border border-dark-tertiary/20 text-xs">
                            <span className="text-[10px]  text-gray-400 uppercase tracking-wider block">Clinical Notes</span>
