...
export default function SSLCertificatesPage() {
  ...
  const columns = [
    { header: 'Domain/Subdomain', accessor: 'domainsubdomain', sortable: true },
    { header: 'Issued By', accessor: 'issuedby', sortable: true },
    { header: 'Valid From', accessor: 'validfrom', sortable: true },
    { header: 'Valid To', accessor: 'validto', sortable: true },
    { header: 'Days Remaining', accessor: 'daysremaining', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => {
  ...
  const mockData = Array.from({ length: 5 }).map((_, i) => {
    return {
      domainsubdomain: i === 0 ? 'api.hospital.com' : 'portal.hospital.com', issuedby: 'Let's Encrypt', validfrom: 'Jan 2026', validto: i===0?'Next Week':'Jan 2027', daysremaining: i===0?'12 Days':'300 Days', status: i===0?'Expiring Soon':'Valid' };
    });
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Tracking expiry of domain certificates to prevent application downtime.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active Certificates</p>
        ...
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Auto-Renew Enabled</p>
          <p className={"text-2xl font-bold mt-2 " + "text-emerald-400"}>10</p>
        </div>
      </div>
