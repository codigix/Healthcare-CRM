...
export default function AmbulanceListPage() {
   ...
   const filteredAmbulances = ambulances.filter((ambulance) => {
      const matchesSearch =
         ...
         false ||
         ambulance.driverName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
         false;
      const matchesStatus =
         statusFilter === "all" ||
         ambulance.status?.toLowerCase() === statusFilter.toLowerCase();
   ...
   const onCallCount = ambulances.filter(
      (a) =>
         a.status?.toLowerCase().includes("dispatched") ||
         a.status?.toLowerCase().includes("on call")
   ).length;
   ...
   const columns_0: Column<any>[] = [
      ...
      {
         header: "Driver", accessor: (ambulance) => (<>\n<span className="text-gray-300">
            {ambulance.driverName}
         </span>\n</>)
      },
      {
         header: "Phone", accessor: (ambulance) => (<>\n<span className="text-gray-300">
            {ambulance.driverPhone}
         </span>\n</>)
      },
      {
         header: "Location", accessor: (ambulance) => (<>\n<div className="flex items-center gap-2">
            <MapPin size={16} className="text-gray-400" />
            <span className="text-gray-300">
               {ambulance.location || "N/A"}
            </span>
         </div>\n</>)
      },
      {
         header: "Actions", accessor: (ambulance) => (<>\n<div className="flex items-center justify-center">
            <button
               onClick={() =>
                  setOpenMenuId(
            ...
            {openMenuId === ambulance.id && (
               <div className="absolute right-0 mt-2 w-40 bg-dark-secondary border border-dark-tertiary rounded shadow-lg z-10 top-full">
                  <Link
                     href={`/ambulance/details?id=${ambulance.id}`}
                  >
                  ...
                  <Link href={`/ambulance/edit/${ambulance.id}`}>
                     <button className="w-full text-left p-2 hover:bg-dark-tertiary transition-colors flex items-center gap-2 text-gray-300 hover:text-white">
                        <Edit size={16} />
                        Edit
                     </button>
                  </Link>
      ...
      <>
         <div className="space-y-6">
            <div className="flex justify-between items-center my-3 my-3">
               <div>
                  <h1 className="text-3xl  mb-2">Ambulance List</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
               <div className="card">
                  <div className="flex items-center gap-2">
                     ...
                     <div>
                        <div className="text-2xl ">{totalAmbulances}</div>
                        <div className="text-mdtext-gray-400">Total Ambulances</div>
               ...
               <div className="card">
                  <div className="flex items-center gap-2">
                     ...
                     <div>
                        ...
                        <div className="text-mdtext-gray-400">Available Ambulances</div>
                        <div className="text-xs text-gray-500">
                           2 on call, 1 in maintenance
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div className="card">
                  <div className="flex items-center gap-2">
                     ...
                     <div>
                        <div className="text-2xl ">{maintenanceCount}</div>
                        <div className="text-mdtext-gray-400">Maintenance Due</div>
                        <div className="text-xs text-orange-500">
                           Next scheduled: May 20, 2023
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div className="card">
                  <div className="flex items-center gap-2">
                     ...
                     <div>
                        <div className="text-2xl ">{onCallCount}</div>
                        <div className="text-mdtext-gray-400">On Call</div>
            ...
            <div className="card">
               ...
               {!loading && filteredAmbulances.length === 0 && (
                  <div className="flex justify-center items-center py-8">
                     <div className="text-gray-400">
                        No ambulances found.{" "}
                        {ambulances.length === 0
                           ? "Add a new ambulance to get started."
                           : ""}
                     </div>
                  </div>
               )}
               ...
               {!loading && filteredAmbulances.length > 0 && (
                  <div className="overflow-x-auto">
                     <DataTable columns={columns_0} data={filteredAmbulances} enableLocalSearch enableLocalPagination />
                  </div>
               )}
