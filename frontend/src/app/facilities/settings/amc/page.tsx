...
export default function AMCSettingsPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { header: 'Rule Name', accessor: 'rulename', sortable: true },
    { header: 'Trigger Condition', accessor: 'triggercondition', sortable: true },
    { header: 'Action', accessor: 'action', sortable: true },
    { header: 'Notified Roles', accessor: 'notifiedroles', sortable: true },
    { header: 'Escalation', accessor: 'escalation', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Alert configurations (30/60/90 days) for contract renewals and vendor blacklisting rules.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Active Rules</p>
          <p className={"text-2xl font-bold mt-2 " + "text-blue-400"}>8</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Alerts Triggered</p>
          <p className={"text-2xl font-bold mt-2 " + "text-amber-400"}>15 (Mo)</p>
        </div>
        <div className="bg-dark-secondary rounded-xl p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Vendor Portal</p>
          <p className={"text-2xl font-bold mt-2 " + "text-emerald-400"}>Enabled</p>
        </div>
      </div>
