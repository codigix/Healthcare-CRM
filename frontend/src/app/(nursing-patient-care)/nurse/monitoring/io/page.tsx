...
export default function IntakeOutputIOPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const columns = [
    { header: 'I/O ID', accessor: 'ioid', sortable: true },
    { header: 'Patient Name', accessor: 'patientname', sortable: true },
    { header: 'Shift', accessor: 'shift', sortable: true },
    { header: 'Total Intake (ml)', accessor: 'totalintakeml', sortable: true },
    { header: 'Total Output (ml)', accessor: 'totaloutputml', sortable: true },
    {
      header: 'Status',
      accessor: (row: any) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center my-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Track patient fluid balance, intake, and output volumes.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-dark-secondary rounded p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Patients Tracked</p>
          <p className={"text-2xl font-bold mt-2 " + "text-blue-400"}>12</p>
        </div>
        <div className="bg-dark-secondary rounded p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Negative Balance</p>
          <p className={"text-2xl font-bold mt-2 " + "text-amber-400"}>2</p>
        </div>
        <div className="bg-dark-secondary rounded p-4 border border-gray-800">
          <p className="text-sm text-gray-400 font-medium">Target Met</p>
