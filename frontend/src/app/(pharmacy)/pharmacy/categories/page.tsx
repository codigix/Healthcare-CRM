...
import { Pill, ArrowLeft, RefreshCw, Loader, Eye, ChevronRight } from "lucide-react";
    ...
    const fetchCategories = async () => {
        try {
            ...
            medicines.forEach((m: any) => {
                const cat = m.category || "General Medications";
                if (!map[cat]) {
            ...
            setError("Failed to fetch medication categories");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center my-3 my-3">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl ">Medicine Categories</h1>
                        <p className="text-gray-400">Classify medications, monitor active catalogs, and track safety level alerts</p>
                    </div>
                </div>
                <button
            ...
            {loading ? (
                ...
                <div className="card text-center py-16">
                    <Pill className="mx-auto text-gray-500 mb-4" size={48} />
                    <h3 className="text-xl  text-white mb-2">No Categories Active</h3>
                    <p className="text-gray-400 max-w-sm mx-auto">
                        You currently have no medicines registered. Add new medications to automatically build categories.
                    </p>
                </div>
            ) : (
                        ...
                        <div key={cat.name} className="card relative overflow-hidden flex flex-col justify-between hover:border-[#1abc9c]/30 transition-all border border-white/5">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-start">
                                    <div className="w-10 h-10 bg-[#1abc9c]/10 rounded flex items-center justify-center text-[#1abc9c]">
                                        <Pill size={20} />
                                    </div>
                                    {cat.lowStockCount > 0 && (
                                        <span className="px-2.5 py-1 bg-red-500/10 text-red-500 border border-red-500/20 rounded-full text-xs font-semibold">
                                ...
                                <div>
                                    <h3 className="text-xl  text-white mb-1">{cat.name}</h3>
                                    <span className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Medication Category</span>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 py-2 border-y border-white/5 text-sm">
                                    <div>
                                        ...
                                        <span className="text-white ">{cat.totalItems} Drugs</span>
                                    </div>
                                    <div>
                                        <span className="text-gray-500 block text-xs">Total In Stock</span>
                                        <span className="text-white ">{cat.totalStock} Units</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-6 pt-4 border-t border-white/5">
                                <Link
                                    href={`/pharmacy/medicines?category=${encodeURIComponent(cat.name)}`}
