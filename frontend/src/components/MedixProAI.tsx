...
export default function MedixProAI() {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <div
                className={`fixed top-0 right-0 h-screen w-full sm:w-96 bg-gradient-to-b from-slate-900 to-slate-950 border-l border-border shadow-2xl transform transition-transform duration-300 z-50 flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'
                    }`}
                ref={sidebarRef}
            >
                {/* Header */}
                <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-4 flex items-center justify-between flex-shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-white  text-lg">MedixPro AI</h2>
                    </div>
                    <button
                        onClick={() => setIsOpen(false)}
                        suppressHydrationWarning
                        className="text-white hover:bg-card/10 p-1 rounded transition-colors"
                    >
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        >
                            <div
                                className={`rounded p-2 ${message.type === 'user'
                                    ...
                                    : 'bg-muted text-gray-100 rounded-bl-none border border-border max-w-2xl'
                                    }`}
                            >
                                ...
                                {message.result?.roomAllocationDetails && (
                                    <div className="mt-4 space-y-3">
                                        <div className="bg-muted/50 rounded p-3 border border-blue-500/30">
                                                    ...
                                                    <div className="mt-3 pt-3 border-t border-border">
                                                        ...
                                                        <div className="space-y-1">
                                                            {message.result.roomAllocationDetails.patients.map((patient, idx) => (
                                                                <div key={idx} className="bg-muted/50 rounded p-2 text-gray-300 text-xs">
                                ...
                                {message.isDoctorSchedule && message.result?.doctorSchedule && (
                                    <div className="mt-4 space-y-3">
                                        <div className="bg-muted/50 rounded p-3 border border-purple-500/30">
                                                    ...
                                                    <table className="w-full text-xs">
                                                        ...
                                                        <tbody>
                                                            {message.result.doctorSchedule.slots.map((slot, idx) => (
                                                                <tr key={idx} className={`border-b border-border ${slot.status === 'booked' ? 'bg-red-950/20' : 'bg-green-950/20'}`}>
                                ...
                                {message.result && message.result.success && message.result.patientName && (
                                    <div className="mt-3 space-y-2">
                                        {message.result.patientName && (
                                            <div className="bg-muted/50 rounded p-2 text-xs space-y-1 border border-cyan-500/30">
                                        ...
                                        <div className="bg-muted/50 rounded p-2 text-xs space-y-1 border border-green-500/30">
                                            <div className="font-semibold text-green-400 mb-2">✅ Appointment Details</div>
                                            <div className="flex justify-between">
                                                <span className="text-gray-300">Token:</span>
                                                <span className="text-green-400 ">{message.result.tokenNumber}</span>
                                            </div>
                                            <div className="flex justify-between">
                    ...
                    {isLoading && (
                        <div className="flex justify-start">
                            <div className="bg-muted border border-border rounded rounded-bl-none p-2">
                ...
                {/* Input Area */}
                <div className="bg-muted/50 border-t border-border px-4 py-4 flex-shrink-0">
                    <form onSubmit={handleSendMessage} className="flex gap-2">
                        <input
                            ...
                            disabled={isLoading}
                            className="flex-1 p-2 bg-muted border border-border rounded text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all disabled:opacity-50 text-sm"
                        />
                        <button
                            type="submit"
                            suppressHydrationWarning
                            disabled={isLoading || !input.trim()}
                            className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-600/50 text-white p-2 rounded transition-colors disabled:cursor-not-allowed flex items-center justify-center"
                        >
                ...
                {/* Footer */}
                <div className="bg-muted border-t border-border p-2 text-xs text-gray-500 text-center flex-shrink-0">
