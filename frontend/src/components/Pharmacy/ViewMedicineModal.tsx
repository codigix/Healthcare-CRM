    ...
    return (
        ...
        >
            {loading ? (
                ...
                <div className="bg-red-500/10 border border-red-500/20 text-red-500 p-2 rounded text-sm font-sans">
                    {error}
                </div>
            ) : medicine ? (
                <div className="space-y-6 pr-2 font-sans text-xs">
                    ...
                    <div className="border-b border-dark-tertiary pb-4">
                        <h3 className="text-lg  text-white leading-tight">{medicine.name}</h3>
                    ...
                    {medicine.description && (
                        <div className="bg-card/[0.02] p-3 rounded border border-white/5">
                            <span className="text-gray-500  uppercase tracking-wider text-[10px] block">Description</span>
                            <p className="text-gray-300 text-xs mt-1 leading-relaxed">{medicine.description}</p>
                        </div>
                    )}

                    {/* 2. Procurement & Date details */}
                    <div className="grid grid-cols-2 gap-x-4 gap-y-3 bg-card/[0.01] p-3 rounded border border-white/5">
                        <div className="col-span-2 pb-1.5 border-b border-white/5  text-white text-xs flex items-center gap-1.5">
                            <Info size={14} className="text-teal-400" />
                            Procurement & Manufacturing Details
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Manufacturer</p>
                            <p className="text-white mt-0.5">{medicine.manufacturer || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Supplier</p>
                            <p className="text-white mt-0.5">{medicine.supplier || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Mfg Date</p>
                            <p className="text-white mt-0.5">
                                {medicine.manufacturingDate ? new Date(medicine.manufacturingDate).toLocaleDateString() : "N/A"}
                            </p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Expiry Date</p>
                            <p className="text-white mt-0.5">
                                {medicine.expiryDate ? new Date(medicine.expiryDate).toLocaleDateString() : "N/A"}
                            </p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Primary Batch Number</p>
                            <p className="text-white font-mono mt-0.5">{medicine.batchNumber || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Recommended Dosage</p>
                    ...
                    {(medicine.sideEffects || medicine.precautions) && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-card/[0.01] p-3 rounded border border-white/5">
                            <div className="md:col-span-2 pb-1.5 border-b border-white/5  text-white text-xs flex items-center gap-1.5">
                                <ShieldAlert size={14} className="text-amber-500" />
                                Clinical Warnings & Safety Info
                            </div>
                            {medicine.sideEffects && (
                                <div>
                                    <p className="text-gray-500  uppercase tracking-wider text-[10px]">Side Effects</p>
                                    <p className="text-gray-300 mt-1 leading-relaxed whitespace-pre-wrap">{medicine.sideEffects}</p>
                                </div>
                            )}
                            {medicine.precautions && (
                                <div>
                                    <p className="text-gray-500  uppercase tracking-wider text-[10px]">Precautions & Warnings</p>
                    ...
                    <div className="bg-card/[0.01] p-3 rounded border border-white/5">
                        <div className="pb-2 border-b border-white/5  text-white text-xs flex items-center gap-1.5">
                    ...
                    <div className="grid grid-cols-3 gap-x-4 gap-y-3 bg-card/[0.01] p-3 rounded border border-white/5">
                        <div className="col-span-3 pb-1.5 border-b border-white/5  text-white text-xs flex items-center gap-1.5">
                            <ShieldCheck size={14} className="text-blue-400" />
                            Stock Inventory & Pricing Details
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Current Stock</p>
                            <p className="text-emerald-400 font-extrabold mt-0.5 text-sm">{medicine.initialQuantity} units</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Reorder Alert Level</p>
                            <p className="text-white mt-0.5">{medicine.reorderLevel} units</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Max Capacity Level</p>
                            <p className="text-white mt-0.5">{medicine.maximumLevel} units</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Purchase Cost</p>
                            <p className="text-white font-semibold mt-0.5">₹{formatPrice(medicine.purchasePrice)}</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Selling Price</p>
                            <p className="text-white font-semibold mt-0.5">₹{formatPrice(medicine.sellingPrice)}</p>
                        </div>
                        <div>
                            <p className="text-gray-500  uppercase tracking-wider text-[10px]">Tax Rate (%)</p>
                    ...
                    <div className="pt-4 border-t border-dark-tertiary font-sans">
                        <h4 className="text-sm  text-white mb-2">Available Batches</h4>
                        {batches.length === 0 ? (
                            <p className="text-gray-500 text-xs italic font-sans">No active batches in stock.</p>
                        ) : (
                            <div className="border border-dark-tertiary rounded overflow-hidden bg-card/[0.01]">
                                <table className="w-full text-left text-xs border-collapse">
                                    <thead>
                                        <tr className="bg-card/5 border-b border-dark-tertiary text-gray-400 ">
                                    ...
                                    <tbody className="divide-y divide-dark-tertiary text-gray-300">
                                        {batches.map((batch) => (
                                            <tr key={batch.id} className="hover:bg-card/5">
                                                <td className="py-2.5 px-3 font-mono  text-teal-400">{batch.batchNumber}</td>
                                                <td className="py-2.5 px-3 text-center  text-white">{batch.quantity} units</td>
                                                ...
                                                <td className="py-2.5 px-3 text-right  text-emerald-400">₹{formatPrice(batch.sellingPrice)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
