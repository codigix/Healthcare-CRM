            ...
            <form onSubmit={handleSubmit} className="space-y-4 font-sans text-xs">
                ...
                {!medicine ? (
                    <div className="relative searchable-medicine-select">
                        ...
                        {/* Dropdown list */}
                        {dropdownOpen && (
                            <div className="absolute z-50 w-full mt-1 bg-[#1a202c] border border-dark-tertiary rounded shadow-xl max-h-48 overflow-y-auto divide-y divide-dark-tertiary/40">
                                {filteredMedicines.length === 0 ? (
                                    ...
                                    filteredMedicines.map((m) => (
                                        ...
                                        >
                                            <div className=" text-white text-xs">{m.name}</div>
                                            {m.genericName && (
                    ...
                    <div>
                        <p className="text-xs text-gray-400 uppercase ">Medicine Name</p>
                        <p className="text-sm text-white  mt-0.5">{medicine.name}</p>
                    </div>
                )}
                    ...
                    <>
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p className="text-sm text-emerald-400  mt-0.5">{activeMedicine.initialQuantity} units</p>
                            </div>
                            <div>
                                <label className="block text-xs  text-gray-300 mb-1">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={3}
                                className="w-full text-sm bg-dark-tertiary border border-dark-tertiary rounded p-3 outline-none text-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 font-sans"
                            />
                        </div>

                        {error && (
                            <div className="bg-red-500/10 border border-red-500/20 text-red-500 text-xs p-2 rounded font-sans">
                                {error}
                            </div>
                        )}
                        ...
                        <div className="flex gap-3 justify-end pt-3 border-t border-dark-tertiary/20 font-sans">
                            <button
                                type="button"
                                onClick={onClose}
                                className="p-2 bg-dark-tertiary hover:bg-dark-tertiary/70 text-white rounded transition-colors font-semibold text-xs"
                            >
                                Cancel
                            </button>
                            <button
                                type="submit"
                                disabled={loading}
                                className="p-2 bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-600 text-white rounded transition-colors  text-xs flex items-center gap-1 shadow shadow-emerald-600/25"
                            >
                                {loading ? (
                                    <>
